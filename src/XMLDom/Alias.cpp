...
std::map<std::string, std::string> Alias::GetBaseName(
  ...
  for (int i = 0; i < node_list.size(); i++) {
    QString alias_value = node_list.item(i).attributes().namedItem("alias_name").nodeValue();
    double process_value = atof(process_variable.c_str());
    if (fabs(alias_value.toDouble() -  process_value) < 0.000001) {

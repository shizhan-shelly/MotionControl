...
MasterClient::~MasterClient() {
  std::map<DeviceModel, ClientHandler *>::iterator iter;
...
void MasterClient::RegisterClientHandler(DeviceModel device_model,
  ...
  std::map<DeviceModel, ClientHandler *>::iterator iter;
  iter = handlers_.find(device_model);
  if (iter != handlers_.end()) {
    delete iter->second;
  }
  handlers_[device_model] = handler;
}
...
bool MasterClient::SetupConnect(DeviceModel device_model) {
  std::map<DeviceModel, ClientHandler *>::iterator iter;
  iter = handlers_.find(device_model);
  if (iter == handlers_.end()) {
    return false;
